...
srvs=( "firewalld" "sendmail" "postfix" "iscsi" "ntalk" "lpd" "printer" "chargen" "atd" "autofs" "bluetooth" \
        "cups" "pcspd" "smartd" "iscsid" "rhcmcertd" "restorecond" "setroubleshoot" "firstboot" "smb")
...
# 密码复杂度策略 
passwd_safety_set="yes"                      # yes表示设置  
dcredit="dcredit=-1"                         # 至少包含1个数字
lcredit="lcredit=-1"                         # 至少包含一个小写字母
ucredit="ucredit=-1"                         # 至少包含一个大写字母      
ocredit="ocredit=-1"                         # 至少包含一个特殊字符
retry="retry=3"                              # 密码修改可尝试错误次数为3次
minlen="minlen=8"                            # 密码最短长度为8
difok="difok=3"                              # 密码和前次密码至少有3位不同 
...
# 备份文件
file=(/etc/profile /etc/login.defs /etc/security/limits.conf /etc/pam.d/system-auth /etc.pam.d/password-auth /etc/selinux/config /etc/pam.d/su /etc/rc.d/rc.local /etc/chrony.conf /etc/pam.d/crond /etc/ssh/sshd_config /etc/sysconfig/i18n /etc/sysctl.conf /etc/init.d/kudzu /etc/sudoers /etc/crontab /etc/passwd /etc/shadow /etc/host.conf /etc/vsftpd/ftpuser /etc/aliases /etc/vsftpd/vsftpd.conf /etc/audit/auditd.conf)
...
# 密码复杂度策略
if [ "${passwd_safety_set}x" = "yesx" ]; then
    pass=$(cat /etc/pam.d/system-auth | grep pam_pwquality.so | grep ${retry} | grep ${minlen} \
        | egrep "(${dcredit}|${ucredit}|${lcredit}|${ocredit})")
    if [ ! -n "${pass}" ] ;then
        sed -i "/password\s*requisite\s*pam_pwquality.so/c \password requisite pam_pwquality.so try_first_pass \
        ${retry} ${minlen} ${lcredit} ${ucredit} ${dcredit} ${ocredit}" /etc/pam.d/system-auth && \
    	echo -e "已完成密码复杂度设置\n" 
    else
        echo -e "密码复杂度设置未作更改\n"
    fi
...
# 禁用ctrl+alt+del组合键
if [ "${ctrlaltdel_set}x" = "yesx" ]; then
    [ -f /usr/lib/systemd/system/ctrl-alt-del.target ] && \
    mv /usr/lib/systemd/system/ctrl-alt-del.target{,.bak$day} && \
    init q && \
    echo "已禁用ctrl+alt+del" || echo "禁用ctrl+alt+del未配置"
    dot_line
fi
...
suidfile=$(find /usr/bin/chage /usr/bin/gpasswd /usr/bin/wall /usr/bin/chfn /usr/bin/chsh /usr/bin/newgrp /usr/bin/write /usr/sbin/usernetctl /usr/sbin/traceroute /bin/mount /bin/umount /bin/ping /sbin/netreport -type f -perm +6000 2>/dev/null)
if [ -n "${suidfile}" ]; then
...
# 检查NTP SERVER
if [ "${chronyd_set}x" = "yesx" ]; then
    default_set=$(cat /etc/chrony.conf | grep -v '^#' | egrep "rhel|centos")
    [[ -n ${default_set} ]] && \
    for i in {0..3}; do
        sed -i "s/server ${i}./# server ${i}./g" /etc/chrony.conf
    done
    sed -i "/# server 3./a\server ${ntpserverip} iburst" /etc/chrony.conf
...
if [ "${anonymous_set}x" = "yesx" ]; then
    if [ ! -f /etc/vsftpd/vsftpd.conf ]; then
        ...
        systemctl restart vsftpd.service && echo -e "限制匿名用户登录ftp配置完成\n"
        dot_line
    fi
fi
